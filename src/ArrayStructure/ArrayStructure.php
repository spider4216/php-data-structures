...
class ArrayStructure implements ArrayInterface
{
    ...
    /**
     * Сортировка методом вставки чуть быстрее чем пузырьковая за
     * счет меньшего количества перестановок, но по
     * О синтаксису все равно также О(N в квадрате)
     *
     * {@inheritDoc}
     * @see \App\Ysiroteno\PhpDataStructures\ArrayStructure\ArrayInterface::insertSort()
     */
    public function insertSort(): void
    {
        /**
         * Прямой цикл, делаем обход по всей структуре
         * чтобы взять каждый элемент, извлечь его
         * и в момент перемещения в право сравнить с
         * перемещаемым
         */
        for ($out = 1; $out < $this->capacity; $out++) {
            /**
             * Извлекаем элемент (копируем в данной реализации)
             * во временную переменную
             */
            $temp = $this->data[$out];

            /**
             * Данная переменная нужна для внутреннего
             * цикла, это нужно для обхода в котором
             * может осуществляться перемещение
             */
            $in = $out;

            /**
             * Поочередно предыдущий элемент больше или равен
             * извлеченному элементу?, если да, то вставки делать не
             * нужно, и осуществить перемещение
             */
            while ($in > 0 && $this->data[$in - 1] >= $temp) {
                /**
                 * Перемещаем вправо предыдущий
                 * элемент
                 */
                $this->data[$in] = $this->data[$in - 1];
                /**
                 * Делаем это до тех пор пока либо не нашли
                 * значение элемента больше чем извлеченное
                 * либо пока не дошли крайней левой точки
                 */
                $in--;
            }

            /**
             * Как только нашли значение больше или равное чем извлеченное
             * или достигли левой точки, перемещаем
             *
             * Конечно может быть такое, что большего значения не будет
             * тогда элемент будет вставлен на место из которого был
             * извлечен
             */
            $this->data[$in] = $temp;
        }
